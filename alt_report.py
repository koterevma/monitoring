...
def main():
    ...
    url = 'https://drive.google.com/u/0/uc?id=1dgdUBBmExKNNZmom_JVUfgHPXcwTN9bk&export=download'
    currbrigade = list()
    ...
    schtab = pd.read_excel("py\sch.ods", engine="odf")
    ...
    for i in range(len(thisday)):
        if (thisday[i]==1)|(thisday[i]==2)|(thisday[i]==5):
            currbrigade.append((i-1,brigades[i-2]))
